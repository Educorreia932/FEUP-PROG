...
double error(double squareroot) {
	return ((squareroot - sqrt(squareroot)) / (sqrt(squareroot))) * 100;
}
...
int main() {
	...
	cout << "The squareroot value of " << n << " is " << rq(n, nMaxIter, delta) << " with an error margin of " << error(rq(n, nMaxIter, delta)) << "%.";
